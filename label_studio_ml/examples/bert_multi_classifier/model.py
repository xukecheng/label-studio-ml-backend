...
class DistilbertClassifier(LabelStudioMLBase):
    """
    ...
    baseline_model_name = os.getenv(
        "BASELINE_MODEL_NAME", "distilbert-base-multilingual-cased"
    )
    ...
    def predict(
        ...
        region = li.get_tag(from_name).label(results)
        pv = PredictionValue(
    ...
    def fit(self, event, data, **additional_params):
        ...
        if (
            ...
            logger.info(
                f"Skip training: the number of tasks is not divisible by {self.START_TRAINING_EACH_N_UPDATES}"
            )
            return
        ...
        config = AutoConfig.from_pretrained(
            self.baseline_model_name, num_labels=len(labels)
        )
        logger.debug(f"Config: {config}")
        model = AutoModelForSequenceClassification.from_pretrained(
            self.baseline_model_name, config=config
        )
